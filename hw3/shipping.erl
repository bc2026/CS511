...
get_occupied_docks(Shipping_State, Port_ID) ->
    ...
    case lists:filter(fun({Key, _, _}) -> Key =:= Port_ID end, LocationsList) of
        false -> throw(error);
        L -> get_occupied_docks_helper(L)
    end.
...
get_container_weight_helper(_Shipping_State, []) ->
    0;

get_container_weight_helper(Shipping_State, [H | T]) ->
    case get_container(Shipping_State, H) of
        {container, H, Weight} -> Weight + get_container_weight_helper(Shipping_State, T),
