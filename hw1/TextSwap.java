...
//        Swapper s = new Swapper(null, content, null, 0);
        ...
        for (int i = 0; i < labels.size(); i++) {
            ...
            if(currentInterval != null) {
                buffer = content.toCharArray(); // Create a shared buffer
        ...
        for (Thread thread : T) {
            thread.join(); // Ensures all threads finish before returning the buffer
        }
    ...
    private static void writeToFile(String contents, int chunkSize, int numChunks) throws Exception {
     ...
     public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            if(contents.length() % chunkSize != 0) throw new Exception("File size must be a multiple of the chunk size");
